...
if command -v yq >/dev/null 2>&1; then
  seam="$(yq '.runs.steps[0].env.CHANGED_PATHS // "MISSING"' "${root}/actions/guard-installed-skill-edits/action.yaml")"
...
grep -qF -- 'git --no-replace-objects --literal-pathspecs diff' \
  "${root}/actions/guard-installed-skill-edits/guard-installed-skill-edits.sh" \
