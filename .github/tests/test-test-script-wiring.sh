...
mkdir -p "$work/repo/.github/workflows" "$work/repo/.github/tests" "$work/repo/actions/fixture-action"
touch "$work/repo/actions/fixture-action/action.yaml"
cat >"$work/base.yaml" <<'YAML'
jobs:
  tests:
    steps:
      - uses: ./actions/fixture-action
      - run: echo baseline
  ci-required-checks:
    needs: [tests]
    steps:
