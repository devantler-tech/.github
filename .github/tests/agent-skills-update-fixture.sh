...
for job in test-update-agent-skills-noop test-update-agent-skills-dry-run; do
  setup_steps=$(yq -r ".jobs.\"$job\".steps // [] | map(select(.uses == \"./actions/setup-agent-skills\")) | length" "$workflow")
  ...
  update_steps=$(yq -r ".jobs.\"$job\".steps // [] | map(select(.uses == \"./actions/update-agent-skills\")) | length" "$workflow")
...
dry_run_unpin=$(yq -r '.jobs."test-update-agent-skills-dry-run".steps[] | select(.uses == "./actions/update-agent-skills") | .with.unpin' "$workflow")
dry_run_mode=$(yq -r '.jobs."test-update-agent-skills-dry-run".steps[] | select(.uses == "./actions/update-agent-skills") | .with."dry-run"' "$workflow")
