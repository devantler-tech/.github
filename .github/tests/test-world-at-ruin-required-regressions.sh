...
if [ ! -x "${resolver}" ]; then
	...
	if resolve "devantler-tech/.github" "pull_request" "${pull_request_sha}" ""; then
...
if [ ! -f "${workflow}" ]; then
	...
	if [ "${source_checkout}" != \
		'{"repository":"devantler-tech/.github","ref":"${{ github.workflow_sha }}","path":"trusted-workflow-source"}' ]; then
