...
write_checkout_condition="$(yq -r '
  [.jobs."auto-merge".steps[]
   | select(.name == "📥 Checkout devantler-tech/.github (this workflow'\''s commit)")
   | .if // ""][0]' "$workflow")"
...
# shellcheck disable=SC2016 # GitHub expressions are compared literally.
expected_review_job_group='enable-auto-merge-mutation-${{ github.repository }}-${{ inputs.concurrency-key || (startsWith(github.workflow_ref, '"'"'devantler-tech/.github/.github/workflows/enable-auto-merge.yaml@'"'"') && '"'"'direct'"'"') || ((inputs.enforce-actor-trust || vars.ENFORCE_ACTOR_TRUST == '"'"'true'"'"') && '"'"'actor-trust-legacy'"'"') || github.workflow_ref }}-${{ github.event.pull_request.number || github.event.issue.number || github.run_id }}'
