...
GOWORK=off go -C "$root/actions/validate-shell-pipelines" build -mod=readonly -o "$work/guard" .
...
files=(
  ...
  actions/guard-installed-skill-edits/guard-installed-skill-edits.sh
  actions/update-agent-skills/mark-internal.sh
)
