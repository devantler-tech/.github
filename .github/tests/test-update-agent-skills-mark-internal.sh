#!/usr/bin/env bash
# Contract test for actions/update-agent-skills/mark-internal.sh: every SKILL.md under a directory ends up
...
script="$repo_root/actions/update-agent-skills/mark-internal.sh"
...
action="$repo_root/actions/update-agent-skills/action.yaml"
...
forwarded=$(yq -r '.jobs."update-agent-skills".steps[] | select(.uses == "./.devantler-tech-actions/actions/update-agent-skills") | .with."mark-internal"' "$workflow")
