...
# The two renders are compared by `<Kind>/<name>` (`<Kind>/<namespace>/<name>`
# for a namespaced document); a resource that merely changes, or moves between
# files under the same identity, is not a deletion.
#
...
# make that record wrong. Matching is exact and whole-line (leading whitespace
# and a trailing CR are tolerated); the body is data and is never evaluated.
#
...
# identities <render-file> — one `<Kind>/[<namespace>/]<name>` per document, sorted,
# unique. Refuses a document that has no kind or no name: such a document cannot be
# reconciled by identity, so the comparison would silently ignore it.
identities() {
  local render="$1" ids
  ...
  # as an empty segment instead of the literal string "null".
  ids="$(yq -N 'select(. != null) | [(.kind // ""), (.metadata.namespace // ""), (.metadata.name // "")] | join("/")' "$render")" ||
    die_unknown "yq could not read '$render'"
  [[ -n "$ids" ]] || die_unknown "'$render' renders no documents"
  local line kind ns name
...
# Acknowledged identities, read as whole lines from the body. `tr -d '\r'`
# tolerates a CRLF body; `sed` strips the prefix and surrounding whitespace.
acked="$(tr -d '\r' <"$body_file" |
  sed -n -E 's/^[[:space:]]*Deletion-Acknowledged:[[:space:]]*([^[:space:]]+)[[:space:]]*$/\1/p' |
  sort -u)"
...
if [[ -n "$removed" ]]; then
  ...
  if [[ -n "$missing" ]]; then
    ...
    echo "deploy-deletions: add one line per resource to the pull-request body, exactly:" >&2
